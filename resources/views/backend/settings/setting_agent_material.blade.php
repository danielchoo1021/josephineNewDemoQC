...
<div class="page-header">
    <h1>
        {{ isset($data['backendlang']['backendlang']['Setting_Agent_Materials']) ? $data['backendlang']['backendlang']['Setting_Agent_Materials'] :'' }}
    </h1>
</div>
...
<div class="form-group">
	<div class="row">
		<div class="col-sm-12">
			...
			<div>
				<form method="POST" action="{{ route('uploadMaterial') }}" id="dropzone" enctype="multipart/form-data">
					@csrf
					<div class="fallback">
...
<script type="text/javascript">
    ...
    $('.product-image-list').on('click', '.product-image-thumbnail .delete-image', function(e){
        e.preventDefault();
        var delete_btn = $(this);
        if(confirm("{{ isset($data['backendlang']['backendlang']['Delete_this_file?']) ? $data['backendlang']['backendlang']['Delete_this_file?'] :''}}") == true){
            var url = '{{ route("DeleteMaterial", ":id") }}';
