...
<div class="form-group">
	<div class="row">
		<div class="col-sm-12">
			...
			<div>
				<form method="POST" action="{{ route('uploadTestimonialImage') }}" class="dropzone well" id="dropzone" enctype="multipart/form-data">
					@csrf
					<div class="fallback">
...
<script type="text/javascript">
	var url = '{{ route("LoadTestimonialImage") }}';
	...
	jQuery(function($){
        ...
        try {
            ...
            var myDropzone = Dropzone.options.dropzone =
            {
                ...
                {
                    if(response == 'Max'){
                        toastr.error('{{ isset($data['backendlang']['backendlang']['Maximum_8_Photo']) ? $data['backendlang']['backendlang']['Maximum_8_Photo'] :'' }}');
                    }else{
                        $('.product-image-list .row').html(response);
                    }
                },
    ...
    $('.product-image-list').on('click', '.product-image-thumbnail .delete-image', function(e){
        ...
        if(confirm('{{ isset($data['backendlang']['backendlang']['Delete_This_Image']) ? $data['backendlang']['backendlang']['Delete_This_Image'] :'' }}') == true){
            var url = '{{ route("DeleteTestimonialImage", ":id") }}';
