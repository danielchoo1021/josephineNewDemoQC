...
<div class="row">
	<div class="col-12">
		<form method="POST" action="{{ route('setting_retail_commissions') }}" id="setting-merchant-form">
			@csrf
			<div class="big-parent">
				<div class="form-group">
					<div class="row">
						...
						<div class="col-6">
							<div class="form-group">
								<h5><b> {{ isset($data['backendlang']['backendlang']['Bonus_Amount']) ? $data['backendlang']['backendlang']['Bonus_Amount'] :'' }} (%)</b></h5>
							</div>
						</div>
					</div>
					<div class="child-div">
						@foreach($SettingRetailCommissions as $SettingRetailCommission)
						<div class="form-group child-row">
							<div class="row">
								<div class="col-6">
									<input type="text" name="target[]" class="form-control" placeholder=" {{ isset($data['backendlang']['backendlang']['e.g.']) ? $data['backendlang']['backendlang']['e.g.'] :'' }} 1500" value="{{ $SettingRetailCommission->target }}">
									<input type="hidden" name="lvl_id[]" value="{{ $SettingRetailCommission->id }}" onkeypress="return isNumberKey(event)">
								</div>
								<div class="col-6">
									<input type="text" name="comm_amount[]" class="form-control" placeholder=" {{ isset($data['backendlang']['backendlang']['e.g.']) ? $data['backendlang']['backendlang']['e.g.'] :'' }} 1-100"  value="{{ $SettingRetailCommission->comm_amount }}" onkeypress="return isNumberKey(event)">
								</div>
							</div>
						</div>
						@endforeach
