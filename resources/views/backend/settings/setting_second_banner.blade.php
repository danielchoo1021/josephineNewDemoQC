...
<div class="form-group">
	<div class="row">
		<div class="col-sm-12">
			...
			<div>
				<form method="POST" action="{{ route('uploadSecondBannerImage') }}" class="dropzone well" id="dropzone" enctype="multipart/form-data">
					@csrf
					<div class="fallback">
...
<script type="text/javascript">
	var url = '{{ route("LoadSecondBannerImage") }}';
    ...
    $('.product-image-list').on('click', '.product-image-thumbnail .delete-image', function(e){
        ...
        if(confirm("{{ isset($data['backendlang']['backendlang']['Delete_This_Image']) ? $data['backendlang']['backendlang']['Delete_This_Image'] :'' }}") == true){
            var url = '{{ route("DeleteSecondBannerImage", ":id") }}';
    ...
    $(document).on('change', '.second_banner_url', function(e){
        e.preventDefault();
        var ele = $(this);
        ...
        $.ajax({
           url: '{{ route("changeSecondBannerUrl") }}',
           type: 'post',
           data: fd,
           contentType: false,
           processData: false,
    ...
    function getIdsOfImages() {
        ...
        $('.product-image-thumbnail').each(function(index) {
            a++;
            var mid = $(this).find('.delete-image').data('id');
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    $('.loading-gif').hide();
                    toastr.error("{{ isset($data['backendlang']['backendlang']['Error_Updating']) ? $data['backendlang']['backendlang']['Error_Updating'] :'' }}: " + error);
                }
            });
        });
    }
