...
<div class="page-header">
    <h1>
         {{ isset($data['backendlang']['backendlang']['Setting_Main_Page']) ? $data['backendlang']['backendlang']['Setting_Main_Page'] :'' }}
    </h1>
</div>
...
<div class="form-group">
	<div class="row">
		<div class="col-sm-12">
			...
			<div>
				<form method="POST" action="{{ route('uploadMainPageImage') }}" class="dropzone well" id="dropzone" enctype="multipart/form-data">
					@csrf
					<div class="fallback">
...
<script type="text/javascript">
	var url = '{{ route("LoadMainPageImage") }}';
    ...
    $('.product-image-list').on('click', '.product-image-thumbnail .delete-image', function(e){
        ...
        if(confirm("{{ isset($data['backendlang']['backendlang']['Delete_This_Image']) ? $data['backendlang']['backendlang']['Delete_This_Image'] :'' }}") == true){
            var url = '{{ route("DeleteMainPageImage", ":id") }}';
    ...
    function getIdsOfImages() { 
        ...
        var fd = new FormData();
            fd.append('mid', arrayA);
            fd.append('number', arrayB);

        $.ajax({
           url: '{{ route("SortMainImage") }}',
           type: 'post',
           data: fd,
           contentType: false,
           processData: false,
    ...
    $('.product-image-list').on('change', '.title', function(){
    	var ele = $(this);

    	var mid = ele.closest('.product-image-thumbnail').find('.delete-image').data('id');
    ...
    $('.product-image-list').on('change', '.description', function(){
    	var ele = $(this);
