...
<div class="form-group">
	<div class="row">
		<div class="col-sm-12">
			<div class="card mb-4">
				...
				<div class="card-body">
					<div class="form-group">
						...
						<input type="text" id="why_choose_us_eyebrow" class="form-control" placeholder="INSPIRED BY NATURE" value="{{ $data['website_setting']->why_choose_us_eyebrow ?? '' }}">
					</div>
					<div class="form-group">
						<label>Heading</label>
						<input type="text" id="why_choose_us_heading" class="form-control" placeholder="Pure Ingredients, Pure Care" value="{{ $data['website_setting']->why_choose_us_heading ?? '' }}">
					</div>
					<button type="button" class="btn btn-primary btn-sm" id="save_why_choose_us_heading">
			...
			<div>
				<form method="POST" action="{{ route('uploadWhyChooseUsImage') }}" class="dropzone well" id="dropzone" enctype="multipart/form-data">
					@csrf
					<div class="fallback">
...
<script type="text/javascript">
	var url = '{{ route("LoadWhyChooseUsImage") }}';
    ...
    $('#save_why_choose_us_heading').click(function(e){
        e.preventDefault();

        var fd = new FormData();
            fd.append('eyebrow', $('#why_choose_us_eyebrow').val());
            fd.append('heading', $('#why_choose_us_heading').val());

        $.ajax({
           url: '{{ route("updateWhyChooseUsHeading") }}',
           type: 'post',
           data: fd,
           contentType: false,
           processData: false,
    ...
    $('.product-image-list').on('click', '.product-image-thumbnail .delete-image', function(e){
        e.preventDefault();
        var delete_btn = $(this);
        if(confirm("{{ isset($data['backendlang']['backendlang']['Delete_This_Image']) ? $data['backendlang']['backendlang']['Delete_This_Image'] :'Delete this image?' }}") == true){
            var url = '{{ route("DeleteWhyChooseUsImage", ":id") }}';
    ...
    $(document).on('change', '.why_choose_us_title', function(e){
        e.preventDefault();
        var ele = $(this);
        ...
        $.ajax({
           url: '{{ route("changeWhyChooseUsTitle") }}',
           type: 'post',
           data: fd,
           contentType: false,
           processData: false,
    ...
    $(document).on('change', '.why_choose_us_subtitle', function(e){
        e.preventDefault();
        var ele = $(this);
        ...
        $.ajax({
           url: '{{ route("changeWhyChooseUsSubtitle") }}',
           type: 'post',
           data: fd,
           contentType: false,
           processData: false,
