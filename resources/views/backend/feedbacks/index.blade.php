...
@section('content')
<form action="{{ route('feedback.feedbacks.index') }}" method="GET">
<div class="row">
	<div class="col-sm-2">
		<div class="form-group">
			<input type="text" class="form-control" name="title" value="{{ !empty('title') && request('title') ? request('title') : '' }}" placeholder="{{ isset($data['backendlang']['backendlang']['Search_Feedback_Title']) ? $data['backendlang']['backendlang']['Search_Feedback_Title'] :'' }}">
		</div>
	</div>
	...
	<div class="row">
		<div class="col-sm-4">
			<div class="form-group">
				...
				<a href="{{ route('feedback.feedbacks.index') }}" class="btn btn-warning btn-sm">
	...
	<div class="col-12">
		<table class="table table-bordered">
			<thead>
				<tr class="info">
					<th>#</th>
					<th>{{ isset($data['backendlang']['backendlang']['products']) ? $data['backendlang']['backendlang']['products'] :'' }}</th>
			...
			<tbody>
				...
				<tr>
					<td>
						{{ $key+1 }}
						<input type="hidden" class="row_id" value="{{ $feedback->id }}">
					</td>
					<td>
...
>								<span class="badge bg-success">
					...
					<td>
						<a href="{{ route('feedback.feedbacks.edit', $feedback->id) }}">
        ...
        if(message == true){
	        $.ajax({
	           ...
	           success: function(response){
	                ...
	                window.location.href="{{ route('feedback.feedbacks.index') }}";
	           },
	        });        	
        }else{
        	$('.loading-gif').hide();
