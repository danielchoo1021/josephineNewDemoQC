...
<div class="row">
	<div class="col-sm-2">
		<div class="form-group">
			<input type="text" class="form-control" name="code" value="{{ !empty('code') && request('code') ? request('code') : '' }}" placeholder="{{ isset($data['backendlang']['backendlang']['Search_Member_Code']) ? $data['backendlang']['backendlang']['Search_Member_Code'] :'' }}">
		</div>
	</div>
	<div class="col-sm-2">
		<div class="form-group">
			<input type="text" class="form-control" name="member_name" value="{{ !empty('member_name') && request('member_name') ? request('member_name') : '' }}" placeholder="{{ isset($data['backendlang']['backendlang']['Search_Member_Name']) ? $data['backendlang']['backendlang']['Search_Member_Name'] :'' }}">
		</div>
	</div>
</div>
		...
		<div class="col-12">
			<table class="table table-bordered">
				...
				<tbody>
					...
					<tr>
						<td>{{ $key+1 }}
							<input type="hidden" name='mid' value="{{ $corporate->id }}">
						</td>
						<td>{{ $corporate->code }}</td>
						<td>{{ $corporate->f_name }}</td>
						...
						<td>
							<a href="{{ route('member.members.edit', $corporate->id) }}">
							...
							<a href="#" class=" change_action red" data-id="98">
								<i class="ace-icon fa fa-ban bigger-130"></i> {{ isset($data['backendlang']['backendlang']['Reject']) ? $data['backendlang']['backendlang']['Reject'] :'' }}
							</a>
							<!-- &nbsp;&nbsp;
							<a href="#" class="red">
...
<script type="text/javascript">
	$('.change_action').click( function(e){
		...
		if(action_id == '1'){
			var action_confirm = confirm("{{ isset($data['backendlang']['backendlang']['Approve_Corporate']) ? $data['backendlang']['backendlang']['Approve_Corporate'] :'' }}");
		}else{
			var action_confirm = confirm("{{ isset($data['backendlang']['backendlang']['Reject_Corporate']) ? $data['backendlang']['backendlang']['Reject_Corporate'] :'' }}");
		}
		if(action_confirm == true){
			$.ajax({
		       url: '{{ route("ApproveRejectCorporate") }}',
		       type: 'post',
		       data: fd,
		       contentType: false,
		       processData: false,
