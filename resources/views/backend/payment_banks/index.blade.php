			...
			<tr>
				...
				<td>{{ isset($data['backendlang']['backendlang']['Bank_Name']) ? $data['backendlang']['backendlang']['Bank_Name'] :'' }}
					...
					@if(empty(request('bank_name_desc')) && empty(request('bank_name_asc')))
						<a href="{{ route('payment_bank.payment_banks.index', ['bank_name_desc=DESC']) }}" 
					...
					@else
						@if(!empty(request('bank_name_desc')))
							<a href="{{ route('payment_bank.payment_banks.index', ['bank_name_asc=ASC']) }}" 
				...
				<td>{{ isset($data['backendlang']['backendlang']['Bank_Holder_Name']) ? $data['backendlang']['backendlang']['Bank_Holder_Name'] :'' }}
					<br>
					@if(empty(request('bank_holder_name_desc')) && empty(request('bank_holder_name_asc')))
						<a href="{{ route('payment_bank.payment_banks.index', ['bank_holder_name_desc=DESC']) }}" 
						   class="{{ !empty(request('bank_holder_name_desc')) ? 'selected' : '' }}">
							<i class="fa fa-sort"></i>
							<input type="hidden" name="sort_data" value="0">
						</a>
					@else
						@if(!empty(request('bank_holder_name_desc')))
							<a href="{{ route('payment_bank.payment_banks.index', ['bank_holder_name_asc=ASC']) }}" 
							   class="{{ !empty(request('bank_holder_name_asc')) ? 'selected' : '' }}">
								<i class="fa fa-sort"></i>
								<input type="hidden" name="sort_data" value="1">
							</a>
						@elseif(!empty(request('bank_holder_name_asc')))
							<a href="{{ route('payment_bank.payment_banks.index', ['bank_holder_name_desc=DESC']) }}" 
							   class="{{ !empty(request('bank_holder_name_desc')) ? 'selected' : '' }}">
								<i class="fa fa-sort"></i>
								<input type="hidden" name="sort_data" value="0">
				...
				<td>{{ isset($data['backendlang']['backendlang']['Bank_Account']) ? $data['backendlang']['backendlang']['Bank_Account'] :'' }}
					<br>
					@if(empty(request('bank_account_desc')) && empty(request('bank_account_asc')))
						<a href="{{ route('payment_bank.payment_banks.index', ['bank_account_desc=DESC']) }}" 
						   class="{{ !empty(request('bank_account_desc')) ? 'selected' : '' }}">
							<i class="fa fa-sort"></i>
							<input type="hidden" name="sort_data" value="0">
						</a>
					@else
						@if(!empty(request('bank_account_desc')))
							<a href="{{ route('payment_bank.payment_banks.index', ['bank_account_asc=ASC']) }}" 
							   class="{{ !empty(request('bank_account_asc')) ? 'selected' : '' }}">
								<i class="fa fa-sort"></i>
								<input type="hidden" name="sort_data" value="1">
							</a>
						@elseif(!empty(request('bank_account_asc')))
							<a href="{{ route('payment_bank.payment_banks.index', ['bank_account_desc=DESC']) }}" 
							   class="{{ !empty(request('bank_account_desc')) ? 'selected' : '' }}">
								<i class="fa fa-sort"></i>
								<input type="hidden" name="sort_data" value="0">
				...
				<td>{{ isset($data['backendlang']['backendlang']['Default']) ? $data['backendlang']['backendlang']['Default'] :'' }}</td>
				<td></td>
			</tr>
			@if(!$payment_banks->isEmpty())
				@foreach($payment_banks as $key => $bank)
				<tr>
					<td>{{ $key+1 }}</td>
					<td>
					...
					<td>
						<a href="{{ route('payment_bank.payment_banks.edit', $bank->id) }}">
...
<script type="text/javascript">
	$('.change-status').click(function(){
        $('.loading-gif').show();
        var ele = $(this);
        var row_id = ele.closest('tr').find('input[name="bid"]').val();
