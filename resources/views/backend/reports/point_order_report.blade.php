...
<div class="container-box form-group">
	...
	<hr>
	<form action="{{ route('point_order_report') }}" method="GET">
			...
			<div class="row">
				<div class="col-sm-4">
					<div class="form-group">
						...
						<a href="{{ route('point_order_report') }}" class="btn btn-warning btn-sm">
	...
	<div class="form-group">
		...
		<span class="badge bg-success" style="font-size: 1rem; padding: 10px;">
			{{ isset($data['backendlang']['backendlang']['Grand_Total_Points']) ? $data['backendlang']['backendlang']['Grand_Total_Points'] :'' }}: <span class="grandTotal"></span>
		</span>
		|
		<a href="{{ route('point_order_report', ['agent='.(!empty(request('agent')) ? request('agent') : ''),'per_page='.(!empty(request('per_page')) ? request('per_page') : ''),
												'today='.$dailyDate]) }}"
			...
			<br>
			{{ number_format($dailySales->dailySales, 2) }} {{ isset($data['backendlang']['backendlang']['Points']) ? $data['backendlang']['backendlang']['Points'] :'' }}
		</a>
		<a href="{{ route('point_order_report', ['agent='.(!empty(request('agent')) ? request('agent') : ''),'per_page='.(!empty(request('per_page')) ? request('per_page') : ''),
			...
			<br>
			{{ number_format($monthlySales->monthlySales, 2) }} {{ isset($data['backendlang']['backendlang']['Points']) ? $data['backendlang']['backendlang']['Points'] :'' }}
		</a>
		<a href="{{ route('point_order_report', ['agent='.(!empty(request('agent')) ? request('agent') : ''),'per_page='.(!empty(request('per_page')) ? request('per_page') : ''),
			...
			<br>
			{{ number_format($yearlySales->yearlySales, 2) }}  {{ isset($data['backendlang']['backendlang']['Points']) ? $data['backendlang']['backendlang']['Points'] :'' }}
		</a>
	</div>
</div>
...
<div class="container-box form-group">
	<div class="form-group" align="right">
		<a href="{{ route('print_point_order_report', ['dates='.(!empty(request('dates')) ? request('dates') : $startDate.' - '.$endDate),
		...
		<a href="{{ route('exportPointOrder', ['dates='.(!empty(request('dates')) ? request('dates') : $startDate.' - '.$endDate),
												 'transaction_no='.(!empty(request('transaction_no')) ? request('transaction_no') : ''),
												 'buyer='.(!empty(request('buyer')) ? request('buyer') : '' ),
												 'item_code='.(!empty(request('item_code')) ? request('item_code') : '' ),
												 'product_code='.(!empty(request('product_code')) ? request('product_code') : '' ),
