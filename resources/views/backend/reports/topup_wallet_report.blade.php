...
<div class="container-box form-group">
	...
	<hr>
	<form action="{{ route('topup_wallet_report') }}" method="GET">
	...
	<div class="form-group">
		...
		<a href="{{ route('topup_wallet_report') }}" class="btn btn-warning btn-sm">
...
<div class="container-box form-group">
	<div class="form-group" align="right">
		<a href="{{ route('exportTopupWalletReport', ['dates='.(!empty(request('dates')) ? request('dates') : $startDate.' - '.$endDate),
					...
					<tr class="success">
						...
						<th>{{ isset($data['backendlang']['backendlang']['User_Code']) ? $data['backendlang']['backendlang']['User_Code'] :'' }}
							@if(empty(request('user_code_desc')) && empty(request('user_code_asc')))
								<a href="{{ route('topup_wallet_report', ['user_code_desc=DESC']) }}" 
							...
							@else
								@if(!empty(request('user_code_desc')))
									<a href="{{ route('topup_wallet_report', ['user_code_asc=ASC']) }}" 
						...
						<th>{{ isset($data['backendlang']['backendlang']['Filter']) ? $data['backendlang']['backendlang']['Filter'] :'' }}</th>
					</tr>
				</thead>
				<tbody>
					@php
					...
					<tr>
						...
						<td>
							@if($topupWallet->status != '3')
								<a href="{{route('topup_wallet_report_detail', $topupWallet->userCode)}}">
