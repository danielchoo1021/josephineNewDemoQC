...
<div class="form-group container-box">
	...
	<hr>
	<form action="{{ route('topup_wallet_report_detail', $code) }}" method="GET">
		<div class="row">
			<div class="col-sm-4">
			...
			<div class="row">
				<div class="col-sm-4">
					<div class="form-group">
						...
						<a href="{{ route('topup_wallet_report_detail', $code) }}" class="btn btn-warning btn-sm">
...
<div class="form-group container-box">
	@if(!empty($topupResult) && isset($topupResult[0]['status']))
		@if($topupResult[0]['status'] != '3')
		<h3>{{ $code }}{{ isset($data['backendlang']['backendlang']['S_Topup_Wallet_Report_Detail']) ? $data['backendlang']['backendlang']['S_Topup_Wallet_Report_Detail'] :'' }}</h3>
		@else
		<h3>{{ isset($data['backendlang']['backendlang']['deleted']) ? $data['backendlang']['backendlang']['deleted'] :'' }} - {{ $code }}{{ isset($data['backendlang']['backendlang']['S_Topup_Wallet_Report_Detail']) ? $data['backendlang']['backendlang']['S_Topup_Wallet_Report_Detail'] :'' }}l</h3>
		@endif
	@else
		<h3>{{ $code }}{{ isset($data['backendlang']['backendlang']['S_Topup_Wallet_Report_Detail']) ? $data['backendlang']['backendlang']['S_Topup_Wallet_Report_Detail'] :'' }}</h3>
	@endif
			...
			<table class="table table-bordered">
				<thead>
					<tr class="info">
						...
						<th>{{ isset($data['backendlang']['backendlang']['Wallet_In']) ? $data['backendlang']['backendlang']['Wallet_In'] :'' }} </th>
						<th>{{ isset($data['backendlang']['backendlang']['Wallet_Out']) ? $data['backendlang']['backendlang']['Wallet_Out'] :'' }}</th>
					</tr>
				</thead>
				<tbody>
					@php
					...
					@if(!empty($topupResult))
						...
						@php
							$a++;
							$totalWalletIn += $detail['in'];
							$totalWalletOut += $detail['out'];
						@endphp							
						@endforeach
						@else
						<tr>
							<td colspan="5">{{ $code }}{{ isset($data['backendlang']['backendlang']['No_Result_Found']) ? $data['backendlang']['backendlang']['No_Result_Found'] :'' }}</td>
						</tr>
					@endif
					<tr class="warning">
						<td style=""  colspan="3">
						...
						<td style=" text-align: left;" >
							<b>{{ number_format($totalWalletIn, 2) }}</b>
						</td>
						<td style=" text-align: left;" >
							<b>{{ number_format($totalWalletOut, 2) }}</b>
						</td>							
					</tr>
