...
<div class="container-box form-group">
		...
		<div class="col-sm-2">
			<div class="form-group">
				<select class="form-control" name="status">
					...
					<option {{ (!empty(request('status')) && request('status') == '55') ? 'selected' : '' }} value="55">{{ isset($data['backendlang']['backendlang']['Expired']) ? $data['backendlang']['backendlang']['Expired'] :'' }}</option>
				</select>
			</div>
		</div>
	</div>
	...
	<!-- <div class="row">
		<div class="col-sm-2">
			<div class="form-group">
				<input type="text" class="form-control" name="referrer_code" value="{{ !empty('referrer_code') && request('referrer_code') ? request('referrer_code') : '' }}" placeholder="Search Referral Code">
			</div>
		</div>
		<div class="col-sm-2">
			<div class="form-group">
				<input type="text" class="form-control" name="referrer_name" value="{{ !empty('referrer_name') && request('referrer_name') ? request('referrer_name') : '' }}" placeholder="Search Referral Name">
			</div>
		</div>
	</div> -->
...
<div class="container-box form-group">
	@if(!empty($data['permission']['permission'][Auth::guard($data['userGuardRole'])->user()->permission_lvl]['merchant-insert']))
	<div class="form-group" align="right">
		<a href="{{ route('ExportMerchant', request()->query()) }}"
   			class="btn btn-outline-warning btn-sm">
    			<i class="bi bi-printer"></i>
    			{{ $data['backendlang']['backendlang']['Export'] ?? '' }}
		</a>

		<a href="{{ route('merchant.merchants.create') }}" class="btn btn-outline-success btn-sm">
			<i class="bi bi-plus"></i> {{ isset($data['backendlang']['backendlang']['Add_New_Merchant']) ? $data['backendlang']['backendlang']['Add_New_Merchant'] :'' }}
		</a>
			...
			<table class="table table-bordered">
					...
					<tr class="info">
						...
						<th>{{ isset($data['backendlang']['backendlang']['Email']) ? $data['backendlang']['backendlang']['Email'] :'' }}
							@if(empty(request('email_desc')) && empty(request('email_asc')))
								<a href="{{ route('merchant.merchants.index', ['email_desc=DESC']) }}" 
							...
							@else
								@if(!empty(request('email_desc')))
									<a href="{{ route('merchant.merchants.index', ['email_asc=ASC']) }}" 
						...
						<th>{{ isset($data['backendlang']['backendlang']['Phone']) ? $data['backendlang']['backendlang']['Phone'] :'' }}
							@if(empty(request('phone_desc')) && empty(request('phone_asc')))
								<a href="{{ route('merchant.merchants.index', ['phone_desc=DESC']) }}" 
							...
							@else
								@if(!empty(request('phone_desc')))
									<a href="{{ route('merchant.merchants.index', ['phone_asc=ASC']) }}" 
				...
				<tbody>
					...
					<tr>
						...
						<td>
							@if(!empty($get_merchant_expired_date[$merchant->code]))
						...
						<td>
							@if(!empty($data['permission']['permission'][Auth::guard($data['userGuardRole'])->user()->permission_lvl]['merchant-edit']))
								<a href="{{ route('merchant.merchants.edit', $merchant->id) }}" class=" btn btn-outline-primary btn-sm" title="{{ isset($data['backendlang']['backendlang']['View_Edit']) ? $data['backendlang']['backendlang']['View_Edit'] :'' }}">
							...
							@if(!empty($data['permission']['permission'][Auth::guard($data['userGuardRole'])->user()->permission_lvl]['merchant-delete']))
