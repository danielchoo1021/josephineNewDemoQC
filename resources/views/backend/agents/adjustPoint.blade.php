	    ...
	    <h3>
	        ...
	        <span>
	            <i class="ace-icon fa fa-angle-double-right"></i>
	           	{{ isset($data['backendlang']['backendlang']['Adjust_Point_Wallet']) ? $data['backendlang']['backendlang']['Adjust_Point_Wallet'] :'' }}
	        </span>
	    </h3>
	</div>
	<div class="form-group">
		<span class="badge bg-primary">{{ isset($data['backendlang']['backendlang']['Point_Wallet_Balance']) ? $data['backendlang']['backendlang']['Point_Wallet_Balance'] :'' }}: {{ $GetCashWalletBalance }}</span>
	</div>
	<form method="POST" action="{{ route('adjustPoint', $agent->id) }}">
		@csrf
		@if($errors->any())
...
<div class="container-box">
	...
	<div class="row">
		<div class="col-sm-12">
			<form method="GET" action="{{ route('adjustPoint', $agent->id) }}">
				<div class="form-group">
					<div class="row">
						<div class="col-sm-2">
							<div class="form-group">
								<select class="form-control" name="type">
									...
									<option {{ (!empty(request('type')) && request('type') == '1') ? 'selected' : '' }} value="1">
										{{ isset($data['backendlang']['backendlang']['Increaes']) ? $data['backendlang']['backendlang']['Increase'] :'' }}
									</option>
						...
						<div class="col-sm-4">
							<button class="btn btn-primary btn-sm">
								<i class="bi bi-search"> {{ isset($data['backendlang']['backendlang']['Search']) ? $data['backendlang']['backendlang']['Search'] :'' }} </i>
							</button>
							<a href="{{ route('adjustPoint', $agent->id) }}" class="btn btn-warning btn-sm">
								<i class="bi bi-arrow-clockwise"> {{ isset($data['backendlang']['backendlang']['Clear_Search']) ? $data['backendlang']['backendlang']['Clear_Search'] :'' }} </i>
							</a>
						</div>
					</div>
				</div>
				...
				<table class="table table-bordered">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								@if ($adjust->type == 1)
									{{ $data['backendlang']['backendlang']['Increase'] ?? '' }}
								@else
									{{ $data['backendlang']['backendlang']['Decrease'] ?? '' }}
								@endif
							</td>
							<td>{{ $adjust->amount }}</td>
							<td>{{ $adjust->remark }}</td>
