...
@section('content')
	...
	<form id="filter-form" action="{{ route('tree_details', ['agent_code' => $agent_code, 'g' => $g]) }}" method="GET">
		<div class="row">
			<div class="col-sm-2">
				<div class="form-group">
					<input type="text" class="form-control" name="code"
						value="{{ !empty('code') && request('code') ? request('code') : '' }}"
						placeholder="{{ isset($data['backendlang']['backendlang']['Search_Agent_Code']) ? $data['backendlang']['backendlang']['Search_Agent_Code'] :'' }}">
				</div>
			</div>
			<div class="col-sm-2">
				<div class="form-group">
					<input type="text" class="form-control" name="agent_name"
						value="{{ !empty('agent_name') && request('agent_name') ? request('agent_name') : '' }}"
						placeholder="{{ isset($data['backendlang']['backendlang']['Search_Agent_Name']) ? $data['backendlang']['backendlang']['Search_Agent_Name'] :'' }}">
				</div>
			</div>
	...
	<div class="form-group">
		...
		<a href="{{ route('tree_details', ['agent_code' => $agent_code, 'g' => $g]) }}" class="btn btn-warning btn-sm">
	...
	<div class="row" style="overflow: auto;">
					...
					<tr class="info">
						...
						<th>{{ isset($data['backendlang']['backendlang']['Code']) ? $data['backendlang']['backendlang']['Code'] :'' }}
							@if(empty(request('code_desc')) && empty(request('code_asc')))
								<a href="{{ route('tree_details', ['agent_code' => $agent_code, 'g' => $g, 'code_desc=DESC']) }}" 
							...
							@else
								@if(!empty(request('code_desc')))
									<a href="{{ route('tree_details', ['agent_code' => $agent_code, 'g' => $g, 'code_asc=ASC']) }}" 
						...
						<th>{{ isset($data['backendlang']['backendlang']['Name']) ? $data['backendlang']['backendlang']['Name'] :'' }}
							@if(empty(request('name_desc')) && empty(request('name_asc')))
								<a href="{{ route('tree_details', ['agent_code' => $agent_code, 'g' => $g, 'name_desc=DESC']) }}" 
							...
							@else
								@if(!empty(request('name_desc')))
									<a href="{{ route('tree_details', ['agent_code' => $agent_code, 'g' => $g, 'name_asc=ASC']) }}" 
						...
						<th>{{ isset($data['backendlang']['backendlang']['Level']) ? $data['backendlang']['backendlang']['Level'] :'' }}
							@if(empty(request('lvl_desc')) && empty(request('lvl_asc')))
								<a href="{{ route('tree_details', ['agent_code' => $agent_code, 'g' => $g, 'lvl_desc=DESC']) }}" 
							...
							@else
								@if(!empty(request('lvl_desc')))
									<a href="{{ route('tree_details', ['agent_code' => $agent_code, 'g' => $g, 'lvl_asc=ASC']) }}" 
		...
		>								<span class="badge bg-success">
								...
								<td>
									<a href="{{ route('agent.agents.edit', $merchant->id) }}">
