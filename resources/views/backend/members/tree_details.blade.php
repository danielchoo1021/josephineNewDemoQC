...
<form action="{{ route('member.members.index') }}" method="GET">
	...
	<div class="col-sm-2">
		<div class="form-group">
			<input type="text" class="form-control" name="member_name" value="{{ !empty('company_name') && request('company_name') ? request('company_name') : '' }}" placeholder="{{ isset($data['backendlang']['backendlang']['Search_Member_Name']) ? $data['backendlang']['backendlang']['Search_Member_Name'] :'' }}">
		</div>
	</div>
				...
				<tr>
					...
					<td>
						...
						<a href="#" class="green change-status" data-id="1">
							<i class="ace-icon fa fa-check bigger-130"></i> {{ isset($data['backendlang']['backendlang']['Reaction']) ? $data['backendlang']['backendlang']['Reaction'] :'' }}
						</a>
						@endif
