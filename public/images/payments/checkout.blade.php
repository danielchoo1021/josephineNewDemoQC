				...
				<div class="container-box">
					<div class="form-group">
						<div class="row">
							...
							<div class="col-6" align="right">
								<a href="#" class="btn btn-primary change-address" data-toggle="modal" data-target="#change-address">
									<i class="fa fa-pencil"></i> {{ isset($data['lang']['lang']['change']) ? $data['lang']['lang']['change'] : 'Change' }}
								</a>
							</div>
						</div>
					</div>
					...
					@if(!empty($getUserDetails->get_default_shipping_address->id))
						...
						<div class="form-group">
							<div class="form-group">
								...
								<b>
									{{ $getUserDetails->get_default_shipping_address->f_name }}
								</b>
							</div>
							<div class="form-group">
								<i class="fa fa-map-marker" aria-hidden="true"></i> &nbsp;&nbsp;
								<span>{{ $getUserDetails->get_default_shipping_address->address }},</span> <br> 
								<span class="ml-22">{{ $getUserDetails->get_default_shipping_address->postcode }} {{ $getUserDetails->get_default_shipping_address->city }},</span> <br>
								<span class="ml-22">
							...
							<div class="form-group">
								<i class="fa fa-home" aria-hidden="true"></i> &nbsp; &nbsp; Self Pickup:  <input type="checkbox" name="cod" class="self-pickup" value="1">
							</div>
							<div class="form-group">
								<i class="fa fa-book" aria-hidden="true"></i> &nbsp; &nbsp; {{ isset($data['lang']['lang']['billing_checkbox']) ? $data['lang']['lang']['billing_checkbox'] :'寄付帐单地址与送货地址相同' }}:  <input type="checkbox" name="same_billing_address" class="same-billing-address" value="1" checked>
							</div>
							<!-- <div class="form-group">
								<i class="fa fa-university" aria-hidden="true"></i> &nbsp; &nbsp; {{ isset($data['lang']['lang']['store_in_stock']) ? $data['lang']['lang']['store_in_stock'] :'储存进货仓' }}:  <input type="checkbox" name="store_stock" class="store-in-stock" value="1">
							</div> -->
						</div>
					@endif
					...
					<div class="bill_fill_in" style="display: none;">
						<div class="form-group">
							<div class="row">
								<div class="col-6">
									<h4>{{ isset($data['lang']['lang']['billing_address']) ? $data['lang']['lang']['billing_address'] :'Billing Address' }}</h4>
								</div>
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<input type="text" name="state_bill" class="form-control" placeholder="State *">
						</div>
						<div class="form-group">
							<div class="row">
								<div class="col-sm-6">
						...
						<div class="container-box f-15">
							...
							@php
		                        if(isset($cart->get_product_det->first_image->image) && !empty($cart->get_product_det->first_image->image)){
		                            $image = file_exists($cart->get_product_det->first_image->image) ? $cart->get_product_det->first_image->image : url('images/no-image-available-icon-6.jpg');
		                        }else{
							...
							<div class="form-group cart-detail">
								<div class="row">
									<div class="col-2 order-images">
										...
										<a href="{{ route('details', [$cart->get_product_det->product_name, md5($cart->get_product_det->id)]) }}">
											<img src="{{ (!empty($cart->get_product_det->first_image->image)) ? url($cart->get_product_det->first_image->image) : url('images/no-image-available-icon-61.jpg') }}" style="width: 100%;">
										</a>
									</div>
										...
										<div class="row">
												...
												<div class="form-group">
													...
													<a href="{{ route('details', [$cart->get_product_det->product_name, md5($cart->get_product_det->id)]) }}">
														...
														@if($cart->get_product_det->variation_enable == '1')
															<br>
															{{ isset($data['lang']['lang']['option']) ? $data['lang']['lang']['option'] :'Option' }}: {{ $cart->get_fv_det->variation_name }}
														@endif
														@if($cart->get_product_det->second_variation_enable == '1')
															<br>
															{{ isset($data['lang']['lang']['second_option']) ? $data['lang']['lang']['second_option'] :'Second Option' }}: {{ $cart->get_sv_det->variation_name }}
														@endif
											...
											<div class="col-sm-2" align="center">
												<div class="form-group product_unit_price">
					                                ...
					                                @if(!empty($get_pricing[$cart->id]['product_special_price']))
					                                <br>
					                                <small>
					                                    <del>
					                                ...
					                                @elseif(!empty($get_pricing[$cart->id]['original_customer_price']) && (Auth::guard('admin')->check() || Auth::guard('merchant')->check()))
					                                <br>
					                                <small>
					                                	<del>
					                                		RM {!! $get_pricing[$cart->id]['original_customer_price'] !!}
					                                	</del>
					                                </small>
					                                @endif
												</div>
										    	...
										    	<div class="modal-content">
										      		...
										      		<div class="modal-body">
										        		<div class="input-group">
							                                <input type="text" name="result" class="form-control discount-code" placeholder="Voucher Code"
							                                       value="">
							                            ...
							                            @if(!empty($getUserDetails->get_saved_vouchers))
								                            ...
								                            <div class="form-group" align="left">
								                            	...
								                            	<a href="#" class="claim-voucher" style="display: block; width: 100%;" data-id="{{ $getClaimedPromo->get_voucher_detail->discount_code }}">
								                            		<div class="row">
								                            			<div class="col-3">
								                            				<img src="{{ url(!empty($getClaimedPromo->get_voucher_detail->image) ? $getClaimedPromo->get_voucher_detail->image : 'images/no-image-available-icon-61.jpg') }}" style="width: 70px;">
								                            			</div>
								                            			<div class="col-9">
								                            				...
								                            				Code: {{ $getClaimedPromo->get_voucher_detail->discount_code }}
								                            			</div>
								                            		</div>
								                            	</a>
								                            </div>
										      		...
										      		<button type="button" class="btn btn-secondary close-modal" data-dismiss="modal" style="display: none;">{{ isset($data['lang']['lang']['close']) ? $data['lang']['lang']['close'] :'Close' }}</button>
							...
							@php
								...
								if(!empty($getUserDetails->get_applied_voucher->get_voucher_detail->id)){
									if(empty($getUserDetails->get_applied_voucher->get_voucher_detail->amount_type)){
										$applied_discount_type = "RM 0";
									}elseif($getUserDetails->get_applied_voucher->get_voucher_detail->amount_type == 'Percentage'){
										$applied_discount_type = $getUserDetails->get_applied_voucher->get_voucher_detail->amount."%";
									}else{
										$applied_discount_type = "RM ".$getUserDetails->get_applied_voucher->get_voucher_detail->amount;
									}
								}
							@endphp
							<div class="form-group">
	                        	<div class="success-message-promo green">
	                        		@if(!empty($getUserDetails->get_applied_voucher->get_voucher_detail->free_shipping) && $getUserDetails->get_applied_voucher->get_voucher_detail->free_shipping == '1')
	                        		Applied Voucher - {{ $getUserDetails->get_applied_voucher->get_voucher_detail->discount_code }} (Free Shipping Voucher)
	                        		@else
	                        		{{ (!empty($getUserDetails->get_applied_voucher->get_voucher_detail->id)) ? "Applied Voucher - ".$getUserDetails->get_applied_voucher->get_voucher_detail->discount_code."(".$applied_discount_type.")" : '' }}
	                        		@endif
	                        		@if(!empty($getUserDetails->get_applied_voucher->get_voucher_detail->id))
	                        			<a href="#" class="remove-applied-promo pull-right" data-id="{{ $getUserDetails->get_applied_voucher->id }}">
								...
								if(!empty($getUserDetails->get_applied_voucher->get_voucher_detail->id)){
									if($getUserDetails->get_applied_voucher->get_voucher_detail->free_shipping == '1'){
										...
										if($getUserDetails->get_applied_voucher->get_voucher_detail->amount_type == 'Percentage'){
											$applied_discount_amount = (float) $totalPrice * $getUserDetails->get_applied_voucher->get_voucher_detail->amount / 100;
										}else{
											$applied_discount_amount = $getUserDetails->get_applied_voucher->get_voucher_detail->amount;
										}
									}
								}
							@endphp
							...
							<div class="form-group">
								<input type="hidden" name="discount_code" id="code" value="{{ (!empty($getUserDetails->get_applied_voucher->get_voucher_detail->id)) ? $getUserDetails->get_applied_voucher->get_voucher_detail->promotion_id : '' }}">
							...
							<div class="form-group">
								<div class="row">
									...
									<div class="col-6" align="right" style="font-size: 20px;">
										...
										<b class="grand-total">RM {{ number_format(($totalPrice - $applied_discount_amount + $get_cart_details['total_shipping_fee'] + $processing_fee), 2) }}</b>
										@php
									...
									<div class="widget-toolbar no-border">
										<ul class="nav nav-tabs" id="recent-tab">
											...
											<li class="parent_payment_method">
												<a data-toggle="tab" class="payment_method f-15" data-id="4" href="#rev-payment-tab">
													{{ isset($data['lang']['lang']['online_payment']) ? $data['lang']['lang']['online_payment'] :'Online Payment' }}
												</a>
											</li>

											<li class="parent_payment_method">
												<a data-toggle="tab" class="payment_method f-15" data-id="3" href="#topup-wallet-tab">{{ isset($data['lang']['lang']['topup_wallet']) ? $data['lang']['lang']['topup_wallet'] :'Topup Wallet' }}</a>
											</li>
											...
											<!-- <li class="parent_payment_method">
												<a data-toggle="tab" class="payment_method f-15" data-id="4" href="#cash-wallet-tab">Commission Wallet</a>
											</li> -->
											<!-- <li class="parent_payment_method">
										...
										<div class="tab-content padding-8">
											<div id="online-tab" class="tab-pane ">
												<div class="form-group" align="center">
													<h4>Support By</h4>
													<br>
													<img src="{{ url('images/cropped-Premier-Pay-LOGO-3-1.png') }}" width="150">
												</div>
												...
												<div class="form-group">
													...
													<div class="card border-danger mb-3" style="max-width: 18rem;" align="center">
														<div class="card-body text-danger">
														    <h5 class="card-title">Low Sugar Lab Sdn Bhd</h5>
														    <h5 class="card-title">Hong Leong Bank</h5>
														    <p class="card-text">03600150010</p>
														</div>
													</div>
												</div>
...
<!-- 													<div class="form-group">
												...
												<div class="form-group">
													...
													@else
														<button class="btn btn-primary btn-block cash-wallet-placeorder-btn bg-color"> 
															{{ isset($data['lang']['lang']['place_order_now']) ? $data['lang']['lang']['place_order_now'] :'立即下单' }}
														</button>
													@endif
												</div>
											...
											<div id="topup-wallet-tab" class="tab-pane" align="center">
												<div class="form-group">
													<div class="row">
														<div class="col-12" align="center"> 
															RM 
															<span class="wallet-balance-amount">
																{{ number_format($GetProductWalletBalance, 2) }}
															</span>
															<br>
															<span class="wallet-desc profile-word">Remaining Topup Wallet Balance</span>
														</div>
													</div>
												</div>
												...
												<div class="form-group">
													<b id="error-message-topup-wallet-banks" class="important-text"></b>
												</div>
												
												<div class="form-group">
													<button class="btn btn-primary btn-block topup-wallet-placeorder-btn bg-color"> Place order now </button>
												</div>
											...
											<div id="rev-payment-tab" class="tab-pane" align="center">
												<div class="form-group">
													</div>
													<div class="form-group">
														<div class="row">
															<div class="col-md-4" align="center" style="display: flex; justify-content: center; align-items: center; margin-bottom: 2.5em; flex-direction: column;">
																...
																<label>
																	...
																	<img src="{{ url('images/payments/fpx.png') }}">
																</label>
															</div>
															<!-- <div class="col-md-4" align="center" style="display: flex; justify-content: center; align-items: center; margin-bottom: 2.5em;">
																<label>
																	<input type="radio" name="payment_id" value="15">
																	<img src="{{ url('images/payments/alipay.png') }}">
																</label>
															</div> -->
															<div class="col-md-4" align="center" style="display: flex; justify-content: center; align-items: center; margin-bottom: 2.5em; flex-direction: column;">
																...
																<label>
																	...
																	<img src="{{ url('images/payments/union_pay.png') }}">
																</label>
															</div>
															<div class="col-md-4" align="center" style="display: flex; justify-content: center; align-items: center; margin-bottom: 2.5em; flex-direction: column;">
																...
																<label>
																	...
																	<img src="{{ url('images/payments/png-clipart-mastercard-visa-bank-card-payment-mastercard-text-service.png') }}">
																</label>
															</div>
														</div>
													</div>
														...
														<div class="row">
															...
															<div class="col-md-4" align="center" style="display: flex; justify-content: center; align-items: center; margin-bottom: 2.5em; flex-direction: column;">
																...
																<label>
																	...
																	<img src="{{ url('images/payments/tng.png') }}">
																</label>
															</div>
															<div class="col-md-4" align="center" style="display: flex; justify-content: center; align-items: center; margin-bottom: 2.5em; flex-direction: column;">
																...
																<label>
																	...
																	<img src="{{ url('images/payments/boost.png') }}">
																</label>
															</div>
														</div>
													</div>
													...
													<div class="form-group">
														@if(!empty($_COOKIE['new_guest']) && empty(Auth::guard($data['userGuardRole'])->check()) && empty(Session::get('continue_guest')))
															<button type="button" class="btn btn-primary btn-block bg-color" data-toggle="modal" data-target="#login-required">
															  {{ isset($data['lang']['lang']['place_order_now']) ? $data['lang']['lang']['place_order_now'] :'Place order now' }}
															</button>
														@else
															<button class="btn btn-primary btn-block revpay-placeorder-btn bg-color"> {{ isset($data['lang']['lang']['place_order_now']) ? $data['lang']['lang']['place_order_now'] :'Place order now' }} </button>
														@endif
													</div>
      ...
      <div class="modal-body" style="overflow: auto;">
        <form method="POST" action="{{ route('add_new_address') }}" id="add-new-address-form">
			...
			<div class="form-group">
                <div class="row">
                    ...
                    <div class="col-6">
                        ...
                        <input type="text" class="form-control required-feild" placeholder="ex: 0171234567" name="phone" value="{{ Auth::guard($data['userGuardRole'])->user()->phone }}"  onkeypress="return isNumberKey(event)">
                    </div>
                </div>
            </div>
			<div class="form-group">
			...
			<div class="form-group row">
				<div class="col-sm-12">
					<label for="country">Country <span class="important-text">*</span></label>
            ...
            <div class="form-group">
      		...
      		<div class="form-group">
	      		<button class="btn -red btn-block btn-sm default_btn add-new-address-btn" style="color: #000 !important;">
	        		Submit
	        	</button>
      		</div>
  		</form>
...
<script type="text/javascript">
	...
	$('.revpay-placeorder-btn').click( function(e){
		...
		var grand_total = $('#hidden_grand_total').val();
		var m = '{{ request("m") }}';
		var same_billing_address = $('.same_billing_address').prop('checked');
	...
	function updateQty(qty, cart_id, ele){
		...
		$.ajax({
	       ...
	       success: function(response){
       		...
       		ele.closest('.cart-detail').find('.product_unit_price').html('RM '+parseFloat(parseFloat(response) / parseFloat(qty)).toFixed(2));

       		ele.closest('.cart-detail').find('.product-total-price').html('RM '+parseFloat(response).toFixed(2));
       		ele.closest('.cart-detail').find('.product-total-price').html('RM '+parseFloat(response).toFixed(2));
       		calc();
	       },
	    });
	...
	function calc()
	{
		...
		$.ajax({
	       ...
	       processData: false,
	       success: function(response){
	       		$('.sub-total-display').html(parseFloat(response[0]).toFixed(2));
				$('.sub-total-value').val(response[0]);
				$('.total-weight-display').html(response[1]);
				$('.total-weight').val(response[1]);
				$('.discount_amount').html('RM '+parseFloat(response[2]).toFixed(2));
				$('.hidden_discount').val(response[2]);
				$('.shipping_amount').html('RM '+parseFloat(response[3]).toFixed(2));
				$('.hidden_shipping_amount').val(response[3]);
				$('.grand-total').html('RM '+parseFloat(response[4]).toFixed(2));
